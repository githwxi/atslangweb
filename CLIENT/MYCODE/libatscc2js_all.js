...
** The starting compilation time is: 2014-10-19: 23h:30m
**
*/
...
//
  ...
  while(true) {
    ...
    switch(tmplab) {
      // ATSbranchseq_beg
      case 1: // __atstmplab0
      if(ATSCKptrisnull(tmp2)) { tmplab_js = 4; break; }
      case 2: // __atstmplab1
      tmp3 = tmp2[0];
      tmp4 = tmp2[1];
      tmp5 = env1[0](env1, tmp3);
      if(tmp5) {
        tmp6 = ats2jspre_stream_filter_cloref(tmp4, env1);
        tmpret1 = [tmp3, tmp6];
      } else {
        tmp7 = ats2jspre_stream_filter_cloref(tmp4, env1);
        ATSPMVlazyval_eval(tmp7); tmpret1 = tmp7[1];
      } // endif
      break;
      // ATSbranchseq_end
      // ATSbranchseq_beg
